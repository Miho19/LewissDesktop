...
describe('getLewissFauxwoodTableEntryAsync', () => {
  ...
  const entryInput: { blindType: Blind }[] = [
    { blindType: "Lewis's 50mm Phoenixwood Venetian" },
    { blindType: "Lewis's 63mm Phoenixwood Venetian" }
  ]
  ...
  it.each(entryInput)(
    ...
    async ({ blindType }) => {
      ...
      const result = await getLewisPhoenixwoodTableEntryAsync(
        blindType,
        0,
        windowDisplay,
        room,
      ...
      expect(isLewissPhoenixwoodTableEntry(tableEntry)).toBeTruthy()
